...
`[Match]
Name=<name of the wired network device> <second one if applicable> [<more ...>]_
...
`[Unit]
Requires=sys-subsystem-net-devices-<wireless-device>.devic_
...
_SUBSYSTEM=="rfkill", ENV{RFKILL_NAME}=="phy0", ENV{RFKILL_TYPE}=="wlan", ACTION=="change", ENV{RFKILL_STATE}=="0", RUN+="/usr/bin/systemctl --no-block stop iwd.service"`
